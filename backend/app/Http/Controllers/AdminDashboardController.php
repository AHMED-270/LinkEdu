...
class AdminDashboardController extends Controller
{
    ...
    public function createUser(Request $request)
    {
        ...
        $validated = $request->validate([
            ...
            'niveau_enseignement' => $hasProfesseurNiveau
                ? ['required_if:role,professeur', 'string', Rule::in(['maternelle', 'primaire', 'college', 'lycee'])]
    ...
    public function updateUser(Request $request, $id)
    {
        ...
        try {
            ...
            if ($validated['role'] === 'professeur') {
                ...
                if ($professeur) {
                    $professeur->fill($professeurPayload);
                    $professeur->save();
                } else {
...
<div style=\"margin:0;padding:24px;background:#f5f7fb;font-family:Arial,Helvetica,sans-serif;color:#0f172a;\">
    ...
    public function createMatiere(Request $request)
    {
        ...
        if ($hasMatiereNiveau) {
            $matierePayload['niveau'] = $validated['niveau'];
        }

        $matiere = Matiere::create($matierePayload);
