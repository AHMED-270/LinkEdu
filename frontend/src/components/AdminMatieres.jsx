...
export default function AdminMatieres({ userRole = 'admin' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    const isGeneralType = formData.type === 'generale';
    const generalTypeResult = isGeneralType
      ? buildGeneralTypeCoefficientsPayload(formData.coefficients_by_niveau_code)
      : null;

    if (generalTypeResult?.error) {
      setFormError(generalTypeResult.error);
      return;
    }

    const specificCode = String(formData.specific_niveau_code || '').toLowerCase();
    ...
    if (!isGeneralType) {
      if (!specificCode || !specificLevel) {
        setFormError('Veuillez choisir un niveau specifique valide.');
        return;
      }

      if (isLyceeSpecific) {
        const allowedFilieres = getFilieresForNiveauCode(specificCode);
        if (!formData.lycee_filiere || !allowedFilieres.includes(formData.lycee_filiere)) {
          setFormError('Veuillez choisir une filiere valide pour le niveau specifique selectionne.');
          return;
        }
      }

      const parsedSpecificCoefficient = Number(formData.specific_coefficient);
      if (!Number.isFinite(parsedSpecificCoefficient) || parsedSpecificCoefficient < 0 || parsedSpecificCoefficient > 10) {
        setFormError('Le coefficient specifique doit etre entre 0 et 10.');
        return;
      }
    }
    ...
    const payload = isGeneralType
      ...
      : {
        ...
        coefficient: Number(formData.specific_coefficient),
        coefficients_by_niveau_code: {
          [specificCode]: Number(formData.specific_coefficient),
        },
        lycee_niveau_code: isLyceeSpecific ? specificCode : null,
        lycee_filiere: isLyceeSpecific ? formData.lycee_filiere : null,
      };
                ...
                {formData.type === 'generale' ? (
                    ...
                    {GENERAL_TYPE_NIVEAU_CODES.map((code) => (
                      <div key={`create-${code}`}>
                        ...
                        <input
                          ...
                          min="0"
                          max="10"
                          value={formData.coefficients_by_niveau_code?.[code] ?? ''}
                  ...
                  <>
                    <div className="sm:col-span-3">
                      <label className="text-sm font-medium text-gray-700">Niveau specifique</label>
                      <select
                        value={formData.specific_niveau_code || 'tc'}
                        onChange={(e) => {
                          const nextCode = e.target.value;
                          const isNextLyceeSpecific = isLyceeNiveauCode(nextCode);
                          const filieres = getFilieresForNiveauCode(nextCode);
                          const nextFiliere = isNextLyceeSpecific
                            ? (
                              filieres.includes(formData.lycee_filiere)
                                ? formData.lycee_filiere
                                : (filieres[0] || '')
                            )
                            : '';
                      ...
                      >
                        {SPECIFIC_NIVEAU_OPTIONS.map((option) => (
                          <option key={`create-specific-code-${option.value}`} value={option.value}>{option.label}</option>
                        ))}
                      </select>
                    </div>

                    {isLyceeNiveauCode(formData.specific_niveau_code || '') && (
                      <div className="sm:col-span-3">
                        <label className="text-sm font-medium text-gray-700">Filiere</label>
                        <select
                          value={formData.lycee_filiere || ''}
                          onChange={(e) => setFormData((prev) => ({ ...prev, lycee_filiere: e.target.value }))}
                          required
                          className="mt-1 w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                        >
                          <option value="">Choisir une filiere</option>
                          {getFilieresForNiveauCode(formData.specific_niveau_code || '').map((filiere) => (
                            <option key={`create-filiere-${filiere}`} value={filiere}>{filiere}</option>
                          ))}
                        </select>
                      </div>
                    )}
                    <div className="sm:col-span-2">
                      ...
                      <input
                        ...
                        min="0"
                        max="10"
                        value={formData.specific_coefficient ?? ''}
                        onChange={(e) => setFormData((prev) => ({ ...prev, specific_coefficient: e.target.value }))}
                        required
                        className="mt-1 w-full px-3 py-2 rounded-xl border border-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500"
                      />
                    </div>
